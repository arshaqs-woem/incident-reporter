...
async function createCallLog({ callId, calledNumber, uvCallId }) {
  await query(
    `INSERT INTO call_logs (call_id, called_number, call_status, uv_call_id)
     VALUES ($1, $2, 'in_progress', $3)
     ON CONFLICT (call_id) DO NOTHING`,
    [callId, calledNumber, uvCallId || null]
  );
}
...
async function getCallSummary(callId) {
  const res = await query(`SELECT * FROM call_summaries WHERE call_id = $1`, [callId]);
  return res.rows[0] || null;
}

async function getTranscripts(callId) {
  const res = await query(`SELECT speaker, message, timestamp FROM transcripts WHERE call_id = $1 ORDER BY timestamp ASC`, [callId]);
  return res.rows;
}
...
module.exports = {
  ...
  saveIntent,
  saveTranscript,
  saveCallSummary, getCallSummary, getTranscripts
};
